<!--- First handle security.  if the user is not Authenticated or the user is not in the Administrator role get the login page --->
	...
	if(isDefined("form.action") AND form.action neq "") {
		...
		switch(form.action)
		{
		  case "add":
				...
				if(not isDuplicate) {
					status=Role.add(form.role);
				} else {
					variables.error="That would create a duplicate role.";
				}
				break;
		  case "update":
				status = Role.update(form.id,form.role);
				break;
		  case "delete":
				status = Role.delete(form.role);
				break;
		}
	}
</cfscript>

<cfinvoke component="#Application.cfcroot#.role" method="getRoles" returnvariable="getRoles"></cfinvoke>

<cfif isDefined("variables.error")><h3><cfoutput>#variables.error#</cfoutput></h3></cfif>

<cfif isDefined("variables.status") AND variables.status>
	<h3><cfoutput>Success</cfoutput></h3>
<cfelseif isDefined("variables.status") AND NOT variables.status>
	<h3><cfoutput>Error</cfoutput></h3>
</cfif>
