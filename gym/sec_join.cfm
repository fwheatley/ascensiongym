...
<cfscript>
	if(isDefined("form.email")) {
		person = createObject("component","com.member");
		user = createObject("component","com.security");
		if(person.isDuplicate(form.username,form.email)) {
			variables.message="Either the username or email you entered already exist in the system ... Please Try Again.";
		} else {
			variables.memberID = person.add(form.email,form.firstName,form.lastName,form.streetAddress,form.streetAddress1,form.city,form.state,form.country,form.postalCode);
			variables.userID = user.addMember(form.username,form.password,variables.memberID);
			user.joinRole("Member",variables.securityID);
		}
	}
</cfscript>
...
<cfif isDefined("variables.memberID")>
	...
	<cfscript>
		user.joinRole(Application.dsn,"Administrator",variables.securityID);
	</cfscript>
	</cfif>
</cfif>
...
<cfinvoke component="#Application.cfcRoot#.state" method="get" returnvariable="state"></cfinvoke>
...
<table width="567" border="0" align="center" cellpadding="0" cellspacing="3" id="formContainer">
	<tr> 
		...
		<td width="183"> 
			<input name="firstName"<cfif isDefined("form.firstName")> value="<cfoutput>#form.firstName#</cfoutput>"</cfif> type="text" tabindex="1" title="First Name" size="25" maxlength="50">
		</td>
		<td width="100" nowrap> 
			<div align="right">Last Name</div>
		</td>
		<td width="185"> 
			<input name="lastName" type="text" tabindex="2" title="Last Name" size="25" maxlength="50">
		</td>
	</tr>
	...
	<tr> 
		...
		<td>
			<input name="userName" type="text" tabindex="3" title="Username" size="25" maxlength="50">
		</td>
		<td nowrap>
			<div align="right">Email</div>
		</td>
		<td>
			<input name="email" type="text" tabindex="4" title="Email" size="25" maxlength="50">
		</td>
	</tr>
	...
	<tr> 
		...
		<td>
			<input name="password" type="password" tabindex="5" title="Password" size="25" maxlength="15">
		</td>
		<td nowrap>
			<div align="right">Verify Password</div>
		</td>
		<td>
			<input name="vPassword" type="password" tabindex="6" title="Verify Password" size="25" maxlength="15">
		</td>
	</tr>
	<tr> 
		<td nowrap>
		...
		<td colspan="3">
			<input name="streetAddress" type="text" tabindex="7" title="Street Address" size="73" maxlength="100">
		</td>
	</tr>
	<tr> 
		<td>&nbsp;</td>
		<td colspan="3">
			<input name="streetAddress1" type="text" tabindex="8" title="Street Address" size="73" maxlength="100">
		</td>
	</tr>
	<tr> 
		<td height="24" nowrap> 
		...
		<td>
			<input name="city" type="text" tabindex="9" title="City" size="25" maxlength="50">
		</td>
		<td nowrap>
		...
		<td>
			...
			<cfoutput query="state">
				<OPTION VALUE="#state.ID#"> #state.name# </OPTION>#chr(10)#
			</cfoutput> 
			</SELECT>
		</td>
	</tr>
	<tr> 
		...
		<td>
			...
			<OPTION VALUE="0" selected> Select Country </OPTION>
         	<cfoutput query="country">
         		<OPTION VALUE="#country.ID#"> #country.name# </OPTION>#chr(10)#
         	</cfoutput>
			</SELECT>
		</td>
		...
		<td class="small">
			<input name="postalCode" type="text" tabindex="12" title="Postal Code" size="25" maxlength="50">
		</td>
	</tr>
	<tr> 
		<td colspan="4">
