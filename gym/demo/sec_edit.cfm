...
<!--- if form.memberID exists we need to update that member. --->
<cfscript>
	if(isDefined("form.memberID")) {
		person = createObject("component","com.person");
		temp = person.update(form.memberID,form.email,form.firstName,form.lastName,form.streetAddress,form.streetAddress1,form.city,form.state,form.country,form.postalCode);
		if(isDefined("form.password") AND isDefined("form.vpassword")) {
			user = createObject("component","com.security");
			changePassword = user.changePassword(form.memberID,form.password);
		}
	}
</cfscript>

<!--- Set a local variable that contains the username we want
	information for.  If the user is in the administrator group
	it is possible for url.member to be passed to get information
	about other users. --->
...
<!--- Invoke the member component to get the members data in a query called member. --->

<cfinvoke component="com.person" method="get" returnvariable="person">
	<cfinvokeargument name="username" value="#variables.person#">
</cfinvoke>
...
<br> 
<cfscript>	
    if (IsUserInRole("inquiry")) 
        WriteOutput("inquiry<br><br>"); 
    if (IsUserInRole("citation admin")) 
        WriteOutput("citation admin<br><br>"); 
    ...
    <form action="<cfoutput>#GetURL()#</cfoutput>" method="post" enctype="application/x-www-form-urlencoded" name="Member" onSubmit="return submitForm(this)">
		<input type="hidden" name="memberID" value="<cfoutput>#person.ID#</cfoutput>">
				...
				<tr> 
				  ...
				  <td>
					<SELECT NAME="state" size="1" id="state" style="width:150;" tabindex="10" title="State/Province">
					  ...
					  <cfoutput query="state"><OPTION VALUE="#state.state#"> #state.name# </OPTION>#chr(10)#</cfoutput>
					</SELECT>
					</td>
				</tr>
				<tr> 
